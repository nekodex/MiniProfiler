module Rack
  class MiniProfiler
    module ProfilingMethods
      ...
      def record_active_resource(method, path, elapsed_ms)
        c = current
        return unless c
        c.current_timer.add_active_resource(method, path, elapsed_ms, c.page_struct) if (c && c.current_timer)
      end

      def record_cache(type, key, elapsed_ms)
        c = current
        return unless c
        c.current_timer.add_cache(type, key, elapsed_ms, c.page_struct) if (c && c.current_timer)
      end
