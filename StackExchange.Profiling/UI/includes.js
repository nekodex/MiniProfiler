...
var MiniProfiler = (function () {
    ...
    return {
        ...
        getActiveResourceTimings: function (root) {
            var result = [],
                addToResults = function (timing) {
                    if (timing.SqlTimings) {
                        for (var i = 0, ARTiming; i < timing.ActiveResourceTimings.length; i++) {
                            ARTiming = timing.ActiveResourceTimings[i];

                            // HACK: add info about the parent Timing to each SqlTiming so UI can render
                            ARTiming.ParentTimingName = timing.Name;
                            result.push(ARTiming);
                        }
                    }
        ...
        getCacheTimings: function (root) {
            var result = [],
                addToResults = function (timing) {
                    if (timing.CacheTimings) {
                        for (var i = 0, cacheTiming; i < timing.CacheTimings.length; i++) {
                            cacheTiming = timing.CacheTimings[i];

                            // HACK: add info about the parent Timing to each SqlTiming so UI can render
                            cacheTiming.ParentTimingName = timing.Name;
                            result.push(cacheTiming);
                        }
                    }
